...
const sectionOrder = ["getting-started", "concepts", "integration", "guides"];
...
export function loadPage(section: string, slug: string): DocPage {
  const filePath = fs.existsSync(path.join(contentDir, section, `${slug}.mdx`))
    ? path.join(contentDir, section, `${slug}.mdx`)
    : path.join(contentDir, section, `${slug}.md`);
...
function parseFrontmatter(raw: string): { meta: DocMeta; body: string } {
  const meta: DocMeta = { title: "cache-kit", description: "", order: 999 };
