...
If you're using an AI agent, code generator, or automated tool to integrate cache-kit, see the [Installation for Agents](/getting-started/installation-agents) page for structured, machine-readable setup information.
...
> **📖 For deeper understanding:** See [Core Concepts](/concepts/concepts) for detailed explanations of `CacheExpander` and `CacheService`, including design philosophy, usage patterns, and examples throughout the documentation.
...
- Learn about [Database & ORM compatibility](/integration/database-compatibility)
- Explore [Cache backend options](/integration/backends) in detail
- Review [Serialization formats](/concepts/serialization)
